...
public class Car {
	...
	// => 포함관계가 성립되기 위해서는 참조필드에 인스턴스가 반드시 저장
	...
	public void carDisplay() {
		...
		//포함관계가 성립된 클래스의 메소드 호출
		// => 참조필드에 저장된 인스턴스를 이용하여 메소드 호출
		// => 참조필드에 인스턴스가 저장되지 않은 상태로 메소드를 호출할 경우 NullPointerException 발생
		//NullPointerException : 참조변수에 인스턴스가 없는 상태(null 저장)에서 메소드를 호출한 경우 발생되는 예외(Exception)
