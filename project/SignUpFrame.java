...
public class SignUpFrame extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (src == cancelBtn) {
			...
			} else if (nameTxt.getText().length() >= 30) {
				JOptionPane.showMessageDialog(this, "이름은 30자 이하로 입력해주세요!");

			} else if (myworkplace.getText().length() == 0) {
				JOptionPane.showMessageDialog(this, "근무지를 반드시 입력해주세요!");
			} else if (myworkplace.getText().length() >= 30) {
				JOptionPane.showMessageDialog(this, "근무지 이름은 30자 이하로 입력해주세요!");
			...
			} else if (myPay.getText().length() >= 16) {
				JOptionPane.showMessageDialog(this, "죄송하지만 시급은 16자 이하로 입력해주세요!");
			} else {
