...
//	public int returnCombo(int selectedCombo) {
//		return selectedCombo;
//	}
//	
//	public String returnValue(String selectedValue) {
//		return selectedValue;
//	}
//	
//	public String returnColname( String colname) {
//		return colname;
//	}
...
//		} else
		...
		if (selectedValue.equals(updateValue)) {// 원래 저장되어있던 값 vs 지금 내가 쓴 값
			...
			try {
				if (selectedCombo == 4) {
					worktimee = update_calWorkTime(updateValue, tmp[4]);
...
//                  member.getSalperhour() }); // 사용자가 변경한 값으로 저장되는 부분!!!!!!!!!!
			...
			if (one == -1) {
				...
				if (choice == JOptionPane.OK_OPTION) {
					...
					// true로 뚫려 있는것만 받아서 비교하면될거같은데

					// 다르다면 dao불러서 db에 업댓시키면됨
					// updateMode(selectedCombo, selectedValue, colname);
//					returnCombo(selectedCombo);
