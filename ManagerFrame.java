...
public class ManagerFrame extends JFrame implements ActionListener {
	...
	private void init() {
		...
		// 테이블에 들어갈 데이터(임시)
		String header[] = { "년월일","출근시간", "분", "퇴근시간", "분", "근무시간", "일급" };
		String contents[][] = { { "20200601", "08", "30", "17", "30", "7", "7300원" },
								{"20200602", "08", "30", "17", "30", "7", "7300원"},
								{ "20200609","08", "30", "17", "30", "7", "7300원"}};

		desktopPane = new JDesktopPane();
		desktopPane.setBackground(Color.DARK_GRAY);
		getContentPane().add(desktopPane);
		// desktopPane.setLayout(new BoxLayout(desktopPane, BoxLayout.X_AXIS));
		desktopPane.setBounds(0, 23, 659, 562);
		
		
		//tableModel = null;
		tableModel = new DefaultTableModel(contents, header) { @Override
		...
		lblNewLabel_7.setFont(new Font("나눔바른펜", Font.PLAIN, 18));
		...
		label_6.setFont(new Font("나눔바른펜", Font.PLAIN, 15));
		...
		label_8.setFont(new Font("나눔바른펜", Font.PLAIN, 15));
		...
		label_9.setFont(new Font("나눔바른펜", Font.PLAIN, 15));
		...
		label_12.setFont(new Font("나눔바른펜", Font.PLAIN, 27));
		...
		lb_wortkspace.setFont(new Font("나눔바른펜", Font.BOLD, 15));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (src == initItem) {
			...
			else str = str + selectedMonth;
			
			Calendar cal = new GregorianCalendar(Integer.parseInt(selectedYear), Integer.parseInt(selectedMonth) - 1, 1);
			...
			}else if(todayWorkHourTxt.getText().length() == 0) {//근무시간 안적혀있다면
				JOptionPane.showMessageDialog(this, "근무시간을 적어주세요!");
			}
			tableModel.addRow(new String[] {str, "8", "30" , "13", "00","4.5","8400"});
