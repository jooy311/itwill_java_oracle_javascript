...
public class ManagerFrame extends JFrame implements ActionListener {
	...
	private JComboBox<Year> yearCombo;
	...
	public void labelSetting(String year, String month) {// 사용자가 해당 달 데이터를 추가하면 실시간으로 추가가 되어야함
		...
		String[] str;
		try {
			str = dao.calTotalNow(member.getId(), year, month);
			totalWorkHour.setText(str[0]);
			currentEarned.setText(str[1]);

			int count = dao.calCountworkDay(member.getId(), year, month);
	...
	public void settingCombo(boolean flag) {
		...
		searchYear.setEnabled(flag);
		searchMonth.setEnabled(flag);
	}
	...
	public void setComboEnable(String colname) {// 사용자가 선택한 열값에 해당하는 콤보박스를 true를 만들기 위한 메소드
		if (colname.equals("년")) {
...
//      String time = pattern.format(new Date());
		...
		selectedUpdateBtn.setFont(new Font("나눔바른펜", Font.BOLD, 12));
		panel_8.add(selectedUpdateBtn);

		cancelBtn = new JButton("변경취소");
		cancelBtn.setFont(new Font("맑은 고딕", Font.PLAIN, 12));
		panel_8.add(cancelBtn);

		selectedDelete = new JButton("지정일 삭제");
		selectedDelete.setFont(new Font("Dialog", Font.BOLD, 12));
		panel_8.add(selectedDelete);

		saveBtn = new JButton("\uC800\uC7A5");
		saveBtn.setFont(new Font("Dialog", Font.BOLD, 12));
		panel_8.add(saveBtn);
		...
		undoBtn.setFont(new Font("굴림", Font.BOLD, 12));
		...
		if (src == initItem) {
			...
			if (choice == JOptionPane.OK_OPTION) {
				try {
					...
					this.repaint(member);// 새로고침 -> tablecontroller를 이용해서 새로고침할 수도 있을거같음

				} catch (SQLException e1) {
					System.out.println("초기화하는 과정에서의 오류");
					e1.printStackTrace();
				}
			}
...
//            JOptionPane.showMessageDialog(this, " 日를 선택해주세요!");
//         } 
			if (startHCombo.getSelectedIndex() != -1) {
			...
			if (endMCombo.getSelectedIndex() != -1) { // 선택했다면
			...
			if (endMCombo.getSelectedIndex() == -1) { // 선택하지 않았다면
...
//                  member.getSalperhour() }); // 사용자가 변경한 값으로 저장되는 부분!!!!!!!!!!
				...
				payInfo = new MemberPayInfo(member.getId(), str_workdate, selectedYear, selectedMonth, selectedDate,
						str_starttime, str_endtime, worktime, money);
			...
			if (one == -1) {
				...
				JOptionPane.showMessageDialog(this, "근무일과 일급은 변경할 수 없습니다.!");
				return;
			} else {
				...
				if (choice == JOptionPane.OK_OPTION) {
					tc.getValueAt(one, col);// 사용자가 선택한 값을 얻어온다.
					settingCombo(false);// 일단 다 false로 만들어 준뒤

					// 그 해당 열에대한 콤보박스만 사용가능하게 열어둔다.
					setComboEnable(colname);

					// 선택한 행의 열 데이터를 배열에 담아온다
					String s1 = null, s2 = null, s3 = null, s4 = null;
					String[] tmp = new String[tc.getColumnCount() - 1];
					...
					//이전의 값과 동일하면 동일하다고 메세지 띄우고
					//다르다면 dao불러서 db에 저장시키면됨
